...
async function renderDiagram() {
  ...
  try {
    ...
    const diagramId = `mdp-mermaid-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    ...
    error.value = cause instanceof Error ? cause.message : "Failed to render diagram.";
  }
}
</script>
...
<template>
  <figure class="mdp-mermaid">
    ...
    <div v-if="error" class="mdp-mermaid__error">{{ error }}</div>
  </figure>
</template>
