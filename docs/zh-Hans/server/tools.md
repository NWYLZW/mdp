...
| `skills` | `SkillDescriptor[]` | 是 | 当前注册的 skill 描述列表。 |
...
| 出参字段 | 类型 | 说明 |
| --- | --- | --- |
| `skills` | `IndexedSkillDescriptor[]` | 带 `clientId` 和 `clientName` 的 skill 描述列表。 |
...
| `args` | `Record<string, unknown>` | 否 | 传给 skill handler 的参数。 |
| `auth` | `AuthContext` | 否 | 作为 `callClient.auth` 下发给 client 的认证上下文。 |
...
| --- | --- | --- |
| `ok` | `boolean` | 成功时为 `true`，失败时为 `false`。 |
| `data` | `unknown` | skill handler 返回的 payload。 |
| `error` | `SerializedError` | 仅失败时出现。 |
...
### 5. 拿 prompt 或 workflow，而不是直接函数调用

这时分别用：

- `getPrompt` 处理 prompt template 或 prompt builder
- `callSkills` 处理工作流型 capability
