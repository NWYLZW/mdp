...
vi.mock(
  ...
  () => ({
    ...
    workspace: {
      ...
      findTextInFiles: (...args: Parameters<typeof state.findTextInFiles>) =>
        state.findTextInFiles(...args),
      openTextDocument: (...args: Parameters<typeof state.openTextDocument>) =>
        state.openTextDocument(...args),
    ...
    commands: {
      executeCommand: (...args: Parameters<typeof state.executeCommand>) =>
        state.executeCommand(...args)
    },
    DiagnosticSeverity: {
      Error: 0,
      Warning: 1,
...
function createUri(value: string): MockUri {
  ...
  const fsPath =
    url.protocol === "file:" ? decodeURIComponent(url.pathname) : url.pathname;
